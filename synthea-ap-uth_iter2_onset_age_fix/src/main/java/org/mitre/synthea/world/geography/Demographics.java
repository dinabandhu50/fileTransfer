...
public class Demographics {
  ...
  private RandomCollection<String> smokerDistribution;
  public Map<String, Double> smoker;
  private RandomCollection<String> alcoholicDistribution;
  public Map<String, Double> alcoholic;
  ...
  //Starts Changes for Smoking fix 
  private static final List<String> CSV_SMOKER = Arrays.asList(
	      "SMOKER","NON_SMOKER");

  private static final List<String> CSV_ALCOHOLIC = Arrays.asList(
  ...
  public static Demographics csvLineToDemographics(Map<String,String> line) {
    ...
    //Starts Changes for smoker Attribute
    d.smoker = new HashMap<>();
    for(String smoker : CSV_SMOKER) {
      double percentage = Double.parseDouble(line.get(smoker));
      d.smoker.put(smoker.toLowerCase(), percentage);
    }
    
    //Starts Changes for alcoholic Attribute
    d.alcoholic = new HashMap<>();
    for(String alcoholism : CSV_ALCOHOLIC) {
      double percentage = Double.parseDouble(line.get(alcoholism));
      d.alcoholic.put(alcoholism.toLowerCase(), percentage);
    }
    
    //Ends Changes for smoker Attribute
    return d;
  }
...
//New method for Smoking Attributes--Changes by Amit
  public String pickSmoker(Random random) {
      // lazy-load in case this randomcollection isn't necessary
      if (smokerDistribution == null) {
          smokerDistribution = buildRandomCollectionFromMap(smoker);
      }
      ...
      return smokerDistribution.next(random);
  }
  //New method for Smoking Attributes--Changes by Amit

  public String pickAlcoholic(Random random) {
    if (alcoholicDistribution == null) {
        alcoholicDistribution = buildRandomCollectionFromMap(alcoholic);
    }
    ...
    return alcoholicDistribution.next(random);
  }
}
