...
public final class LifecycleModule extends Module {
  ...
  public static final String BIOMARKER_ALK = "alk";
  public static final String BIOMARKER_PD1 = "pd1";
  public static final String BIOMARKER_KRAS = "kras";
  public static final String BIOMARKER_EGFR = "egfr";
  ...
  public static void birth(Person person, long time) {
    ...
    // Process biomarkers
    attributes.put(BIOMARKER_ALK, getBiomarker(person, BIOMARKER_ALK));
    attributes.put(BIOMARKER_EGFR, getBiomarker(person, BIOMARKER_EGFR));
    attributes.put(BIOMARKER_KRAS, getBiomarker(person, BIOMARKER_KRAS));
    attributes.put(BIOMARKER_PD1, getBiomarker(person, BIOMARKER_PD1));
  ...
  private static void setupVitalSignGenerators(Person person) {
    person.setVitalSign(VitalSign.SYSTOLIC_BLOOD_PRESSURE,
            new BloodPressureValueGenerator(person, BloodPressureValueGenerator.SysDias.SYSTOLIC));
    person.setVitalSign(VitalSign.DIASTOLIC_BLOOD_PRESSURE,
            new BloodPressureValueGenerator(person, BloodPressureValueGenerator.SysDias.DIASTOLIC));
  }

  public static boolean getBiomarker(Person person, String biomarker) {
    return person.rand() < (double) BiometricsConfig.get("biomarkers." + biomarker);
  }
  ...
  private static Map<String, Double> createDrugImpactsMap() {
    ...
    impacts.put("00087606313", -1.5); // metformin
    impacts.put("00169280015", -0.5); // liraglutide
    impacts.put("50458014130", -0.5); // canagliflozin
    impacts.put("00088221905", -3.0); // basal insulin
    impacts.put("00002771299", -6.0); // prandial insulin
  ...
  private static final int[] NORMAL_UREA_NITROGEN_RANGE =
      BiometricsConfig.ints("metabolic.basic_panel.urea_nitrogen.normal");
  private static final int[] CHANCE_TO_ONSET_CKD_NITROGEN_RANGE =
      BiometricsConfig.ints("metabolic.basic_panel.urea_nitrogen.chance_to_onset_ckd");
  private static final int[] MILD_KIDNEY_DAMAGE_UREA_NITROGEN_RANGE =
      BiometricsConfig.ints("metabolic.basic_panel.urea_nitrogen.mild_kidney_damage");
  private static final int[] MODERATE_KIDNEY_DAMAGE_UREA_NITROGEN_RANGE =
      BiometricsConfig.ints("metabolic.basic_panel.urea_nitrogen.moderate_kidney_damage");
  private static final int[] SEVERE_KIDNEY_DAMAGE_UREA_NITROGEN_RANGE =
      BiometricsConfig.ints("metabolic.basic_panel.urea_nitrogen.severe_kidney_damage");
  private static final int[] ESRD_UREA_NITROGEN_RANGE =
      BiometricsConfig.ints("metabolic.basic_panel.urea_nitrogen.esrd");
  ...
  private static final int[] CHANCE_TO_ONSET_CKD_CC_RANGE =
      BiometricsConfig.ints("metabolic.basic_panel.creatinine_clearance.chance_to_onset_ckd");
  ...
  private static void calculateVitalSigns(Person person, long time) {
    ...
    int kidneyDamage = ((Integer) person.attributes.getOrDefault("ckd", 0)) - 1;
    int[] ccRange;
    int[] mcrRange;
    int[] unRange;
    switch (kidneyDamage) {
      case 1:
        ...
        unRange = MILD_KIDNEY_DAMAGE_UREA_NITROGEN_RANGE;
        break;
      case 2:
        ccRange = MODERATE_KIDNEY_DMG_CC_RANGE;
        mcrRange = CONTROLLED_MCR_RANGE;
        unRange = MODERATE_KIDNEY_DAMAGE_UREA_NITROGEN_RANGE;
        break;
      case 3:
        ccRange = SEVERE_KIDNEY_DMG_CC_RANGE;
        mcrRange = UNCONTROLLED_MCR_RANGE;
        unRange = SEVERE_KIDNEY_DAMAGE_UREA_NITROGEN_RANGE;
        break;
      case 4:
        ccRange = ESRD_CC_RANGE;
        mcrRange = PROTEINURIA_MCR_RANGE;
        unRange = ESRD_UREA_NITROGEN_RANGE;
        break;
      default:
        if ((boolean)person.attributes.getOrDefault("chance_to_onset_ckd", false)) {
          ccRange = CHANCE_TO_ONSET_CKD_CC_RANGE;
          unRange = CHANCE_TO_ONSET_CKD_NITROGEN_RANGE;
        } else if ("F".equals(person.attributes.get(Person.GENDER))) {
          ccRange = NORMAL_FEMALE_CC_RANGE;
          unRange = NORMAL_UREA_NITROGEN_RANGE;
        } else {
  ...
  private static void startSmoking(Person person, long time) {
    ...
    if (person.ageInYears(time) == 16 && person.attributes.get(Person.SMOKING_FLAG).equals(true)) {
      //int year = Utilities.getYear(time);
      //Boolean smoker = person.rand() < likelihoodOfBeingASmoker(year);
        person.attributes.put(Person.SMOKER, true);
        person.attributes.put(Person.SMOKER_HISTORY, true);
        double quitSmokingBaseline = Double.parseDouble(Config.get("lifecycle.quit_smoking.baseline", "0.01"));
  ...
  private static void startAlcoholism(Person person, long time) {
    ...
    if ( person.ageInYears(time) == 25 && person.attributes.get(Person.ALCOHOLIC_FLAG).equals(true)) {
    	person.attributes.put(Person.ALCOHOLIC, true);
    	person.attributes.put(Person.ALCOHOLIC_HISTORY, true);
    	double quitAlcoholismBaseline = Double.parseDouble(Config.get("lifecycle.quit_alcoholism.baseline", "0.05"));
  ...
  public static void quitSmoking(Person person, long time) {
    ...
    if (person.attributes.containsKey(Person.SMOKER)) {
      if (person.attributes.get(Person.SMOKER).equals(true) && age >=16) {
  ...
  public static void quitAlcoholism(Person person, long time) {
    ...
    if (person.attributes.containsKey(Person.ALCOHOLIC)) {
      if (person.attributes.get(Person.ALCOHOLIC).equals(true) && age >=25) {
